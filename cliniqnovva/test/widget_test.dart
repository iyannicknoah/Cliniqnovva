...
void main() {
  testWidgets('DashboardScreen placeholder renders under AppTheme', (
    tester,
  ) async {
  ...
  testWidgets(
    ...
    (tester) async {
      await tester.pumpWidget(
        _wrap(CliniqnovvaButton(label: 'Save', onPressed: () {})),
      );
  ...
  testWidgets('AvatarWidget shows gradient initials when no photo is given', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(const AvatarWidget(firstName: 'Alice', lastName: 'Uwase')),
    );
  ...
  testWidgets('MetricCard renders its label and value', (tester) async {
    await tester.pumpWidget(
      _wrap(const MetricCard(value: '128', label: 'Patients Today')),
    );
  ...
  testWidgets(
    'CliniqnovvaSidebar matches the page background in both light and dark theme',
    (tester) async {
      const items = [
        SidebarNavItem(
          label: 'Dashboard',
  ...
  testWidgets(
    'LoginScreen renders the pageBackground scaffold, logo, and both fields',
    (tester) async {
      ...
      expect(
        wordmark.style?.fontWeight!.value,
        greaterThan(welcome.style?.fontWeight!.value ?? 0),
      );
  ...
  testWidgets(
    'LoginScreen password field toggles obscureText via the eye icon',
    (tester) async {
      final router = GoRouter(
        initialLocation: '/login',
        routes: [
      ...
      await tester.tap(
        find.byWidgetPredicate((w) => w is HeroIcon && w.icon == HeroIcons.eye),
      );
      await tester.pump();
  ...
  testWidgets(
    'SuspendedScreen shows the suspension message and a Sign Out button',
    (tester) async {
      await tester.pumpWidget(
  ...
  testWidgets(
    'CliniqnovvaButton.text renders underlined when requested and responds to tap',
    (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(
  ...
  testWidgets(
    'CliniqnovvaTableHeader renders one column label per cell with dividers',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          const CliniqnovvaTableHeader(
            columns: ['Client', 'Status', 'Revenue', 'Since'],
          ),
        ),
      );
  ...
  testWidgets(
    'CliniqnovvaTableRow lays out one cell per Expanded and responds to tap',
    (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(
  ...
  test(
    ...
    () {
      ...
      expect(org.billingStatus, 'pending');
  ...
  test(
    ...
    () {
      ...
      expect(org.billingStatus, 'notPaid');
    },
  );
  ...
  testWidgets('StatusBadge renders Active/Suspended with the right tone', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(const StatusBadge(text: 'Active', type: BadgeType.success)),
    );
    expect(find.text('Active'), findsOneWidget);

    await tester.pumpWidget(
      _wrap(const StatusBadge(text: 'Suspended', type: BadgeType.error)),
    );
  ...
  testWidgets('No italic text style is used anywhere in the login screen', (
    tester,
  ) async {
    final router = GoRouter(
      initialLocation: '/login',
    ...
    for (final t in allText) {
      expect(
        ...
        reason: 'Found italic text: "${t.data}"',
      );
    }
  });
}
